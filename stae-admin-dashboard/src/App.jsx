...
const styles = {
  ...
  sidebar: { width: '300px', borderRight: '1px solid rgba(255,255,255,0.1)', padding: '40px 24px', display: 'flex', flexDirection: 'column', gap: '40px', background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(20px)' },
...
export default function App() {
  ...
  useEffect(() => {
    ...
    setCandidates([
      ...
      { id: 2, nome: "REGISTO EM ANÁLISE 02", nuit: "123456789", distrito: "DONDO", status: "PENDENTE" }
    ]);
  }, []);
    ...
    <div style={styles.container}>
      <aside style={styles.sidebar}>
        <div>
          ...
          <h2 style={{ fontSize: 18, fontWeight: 900, background: 'linear-gradient(to right, #d4a30d, #fff)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', lineHeight: 1.3 }}>SECRETARIADO TÉCNICO DE ADMINISTRAÇÃO ELEITORAL</h2>
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ padding: '16px 24px', opacity: 0.4, display: 'flex', gap: 16, alignItems: 'center' }}><MapPin size={20}/> Unidades Distritais</div>
          <div style={{ padding: '16px 24px', opacity: 0.4, display: 'flex', gap: 16, alignItems: 'center' }}><FileText size={20}/> Relatórios Pátria</div>
        </nav>
      </aside>

      <main style={styles.main}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 56 }}>
           <div><h1 style={{ fontSize: 32, fontWeight: 900 }}>Gestão Provincial Eleitoral</h1><p style={{ opacity: 0.5, marginTop: 8 }}>República de Moçambique | Painel de Monitorização Oficial</p></div>
           <button style={styles.button}><Plus/> REGISTO PRESENCIAL NO BALCÃO</button>
        </header>

        <div style={{ display: 'flex', gap: 24, marginBottom: 56 }}>
           {[{ label: "INSCRITOS", val: candidates.length, color: "#d4a30d" }, { label: "EM FILA", val: "1", color: "#f59e0b" }, { label: "VALIDADOS", val: "1", color: "#10b981" }].map((s, i) => (
        ...
        <div style={styles.card}>
           <h3 style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 18, marginBottom: 32 }}><ShieldCheck color="#10b981"/> Lista de Candidatos Monitorizada</h3>
           <table style={styles.table}>
              <thead>
                 <tr><th style={styles.th}>Nome Titular</th><th style={styles.th}>Identificação Oficial</th><th style={styles.th}>Município / Distrito</th><th style={styles.th}>Estado</th><th style={styles.th}>Ação</th></tr>
              </thead>
              <tbody>
                 {candidates.map(c => (
                    <tr key={c.id}>
                       ...
                       <td style={styles.td}><button style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid #10b981', color: '#10b981', padding: '6px 12px', borderRadius: 8, cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>VERIFICAR</button></td>
                    </tr>
                 ))}
              </tbody>
           </table>
