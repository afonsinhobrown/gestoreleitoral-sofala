...
export default function App() {
  ...
  useEffect(() => {
    ...
    setCandidates([
      ...
      { id: 2, nome: "INSCRITO NACIONAL TESTE", nuit: "123456789", distrito: "DONDO", posto: "Chapa", localidade: "Dondo-Sede", role: "BRIGADISTA", docs: "PENDENTE", status: "PENDENTE" }
    ]);
  }, []);
    ...
    <div style={styles.container}>
      <aside style={styles.sidebar}>
        <div>
          ...
          <p style={{ fontSize: 10, opacity: 0.4, letterSpacing: 1, fontWeight: 700, marginTop: 10 }}>GESTOR PROVINCIAL DE SOFALA</p>
        </div>
        <nav style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ ...styles.card, background: 'rgba(212,163,13,0.1)', border: '1px solid #d4a30d', display: 'flex', gap: 16, alignItems: 'center' }}><Users color="#d4a30d" size={18}/> <b>Monitoria Nacional</b></div>
      ...
      <main style={styles.main}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 40 }}>
           <div><h1 style={{ fontSize: 26, fontWeight: 900 }}>Processo Eleitoral 2026</h1><p style={{ opacity: 0.5, marginTop: 4 }}>Controlo de Candidaturas, Documentação e Certificados</p></div>
           <button style={styles.button}><Plus/> NOVO REGISTO PRESENCIAL</button>
        </header>

        <div style={{ display: 'flex', gap: 20, marginBottom: 40 }}>
           {[{ l: "MMV", v: "1" }, { l: "BRIGADISTAS", v: "1" }, { l: "DOCUMENTOS OK", v: "1" }, { l: "PENDENTES", v: "1" }].map((s, i) => (
        ...
        <div style={styles.card}>
           <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
              <h3 style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 16 }}><ClipboardCheck color="#10b981"/> Pauta de Candidatos de Sofala</h3>
              <div style={{ padding: '8px 16px', borderRadius: 8, background: 'rgba(255,255,255,0.05)', fontSize: 12 }}>PROCESSO EM TEMPO REAL</div>
           </div>
           
           <table style={styles.table}>
              <thead>
                 <tr><th style={styles.th}>Nome Candidato</th><th style={styles.th}>Função</th><th style={styles.th}>Distrito/Posto</th><th style={styles.th}>Certificado</th><th style={styles.th}>Estado</th><th style={styles.th}>Verificação</th></tr>
              </thead>
              <tbody>
                 {candidates.map(c => (
                    <tr key={c.id}>
                       <td style={styles.td}><b>{c.nome}</b><p style={{ fontSize: 10, opacity: 0.4 }}>NUIT: {c.nuit}</p></td>
                       <td style={styles.td}><span style={{ color: '#d4a30d', fontWeight: 800 }}>{c.role}</span></td>
                       <td style={styles.td}>{c.distrito}<p style={{ fontSize: 10, opacity: 0.4 }}>PA: {c.posto}</p></td>
