...
// API DINÂMICA (SOFALA 2024 PRODUÇÃO)
const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000/api';

// --- MOTOR DE RECORTE MÓVEL (VISÃO AFONSO) ---
  ...
  return (
    <div className="premium-container" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, background: '#000', padding: 0 }}>
       <div style={{ padding: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#001f42' }}>
          ...
          <h3 style={{ fontSize: 16 }}>Enquadrar Foto Oficial</h3>
       </div>
       ...
       <div style={{ padding: '24px 32px', background: '#001f42', textAlign: 'center' }}>
          ...
          <button className="stae-button" onClick={generateImg}>CONFIRMAR FOTO</button>
       </div>
    </div>
  );
};

// --- APP PRINCIPAL (100% DINÂMICO) ---
export default function App() {
  ...
  const cleanOcrResult = (rawText) => {
    ...
    const noisyWords = ['REPUBLICA', 'MOÇAMBIQUE', 'BILHETE', 'IDENTIDADE', 'NAME', 'NOME', 'APELIDO'];
  ...
  const handleFinalSubmit = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/candidate/apply`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...regForm, photo: extractedPhoto })
      });
      setView('portal');
    } catch { alert('Erro na submissão nacional'); }
  ...
  return (
    <AnimatePresence mode='wait'>
      {view === 'landing' && <div className="premium-container" style={{ justifyContent: 'center' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 48 }}><h1 className="gold-gradient-text" style={{ fontSize: 28, marginBottom: 8 }}>STAE MOÇAMBIQUE</h1><p style={{ opacity: 0.6, fontSize: 13, fontWeight: 700 }}>GESTÃO INTEGRADA DO CICLO ELEITORAL</p></div>
        <div style={{ display: 'grid', gap: 16 }}>
          <button className="glass-card" style={{ padding: 24, textAlign: 'left', display: 'flex', gap: 20 }} onClick={() => setView('login')}><User color="var(--stae-gold)"/> <div><h3>Entrar no Portal</h3><p style={{ opacity: 0.4, fontSize: 12 }}>Consultar Pauta e Credencial</p></div></button>
          <button className="glass-card" style={{ padding: 24, textAlign: 'left', display: 'flex', gap: 20 }} onClick={() => setView('register')}><ShieldCheck color="#4ade80"/> <div><h3>Nova Candidatura</h3><p style={{ opacity: 0.4, fontSize: 12 }}>Registo via IA (BI ou NUIT)</p></div></button>
        </div>
      </div>}

      {view === 'register' && <div className="premium-container" style={{ justifyContent: 'center' }}><motion.div initial={{ opacity: 0 }} className="glass-card" style={{ textAlign: 'center' }}><h2 className="gold-gradient-text" style={{ fontSize: 24, marginBottom: 12 }}>IDENTIFICAÇÃO BIOMÉTRICA</h2><p style={{ opacity: 0.6, marginBottom: 40 }}>O sistema irá extrair os seus dados e fotografia oficial a partir do documento moçambicano.</p><label className="stae-button" style={{ display: 'flex', gap: 12, justifyContent: 'center', cursor: 'pointer' }}><Camera /> {loading ? `Detectando... ${ocrProgress}%` : 'Abrir Scanner STAE'}<input type="file" accept="image/*" onChange={handleFileUpload} style={{ display: 'none' }} disabled={loading} /></label></motion.div></div>}
      ...
      {view === 'confirm' && <div className="premium-container">
         <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 32 }}><h2 style={{ fontSize: 20 }}>Verificação de Dados</h2></div>
         <div className="glass-card" style={{ padding: 24, textAlign: 'center', marginBottom: 24 }}><img src={extractedPhoto} width="120" style={{ borderRadius: 20, border: '3px solid var(--stae-gold)' }} /><p style={{ fontSize: 11, opacity: 0.5, marginTop: 12 }}>BIOMETRIA CAPTURADA</p></div>
         <div className="glass-card"><div style={{ display: 'grid', gap: 16 }}><div><p style={{ fontSize: 11, opacity: 0.4 }}>NOME LIDO</p><input type="text" className="stae-input" value={regForm.nome} onChange={e => setRegForm({...regForm, nome: e.target.value})} /></div><div><p style={{ fontSize: 11, opacity: 0.4 }}>Nº BI / NUIT</p><input type="text" className="stae-input" value={regForm.nuit} onChange={e => setRegForm({...regForm, nuit: e.target.value})} /></div></div><button className="stae-button" style={{ marginTop: 24 }} onClick={handleFinalSubmit}>{loading ? 'Submetendo...' : 'Confirmar Inscrição'}</button></div>
      </div>}
      ...
      {view === 'portal' && <div className="premium-container" style={{ justifyContent: 'center' }}>
         <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} style={{ background: 'white', borderRadius: 16, overflow: 'hidden', boxShadow: '0 20px 40px rgba(0,0,0,0.4)', border: '1px solid #ddd' }}>
            ...
            <div style={{ background: '#f8fafc', padding: 16, textAlign: 'center' }}><button className="stae-button" onClick={() => setView('landing')} style={{ background: '#001f42' }}><Download size={18}/> SALVAR CREDENCIAL</button></div>
         </motion.div>
      </div>}

      {view === 'login' && <div className="premium-container" style={{ justifyContent: 'center' }}><div className="glass-card"><h2 style={{ textAlign: 'center', marginBottom: 24 }}>Acesso</h2><input type="email" className="stae-input" placeholder="NUIT ou BI" /><button className="stae-button" onClick={() => setView('landing')}>Voltar</button></div></div>}
    </AnimatePresence>
  );
}
